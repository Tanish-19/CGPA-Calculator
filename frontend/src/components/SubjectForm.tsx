...
type NewSubjectState = {
  name: string;
  marks: string; // keep as string for controlled input, parse when adding
  totalMarks: string;
  credits: string;
  grade: string;
};
...
export const SubjectForm: React.FC<SubjectFormProps> = ({ subjects, onSubjectsChange }) => {
  const [newSubject, setNewSubject] = useState<NewSubjectState>({
    name: '',
    marks: '',
    totalMarks: '100',
    credits: '',
  ...
  const addSubject = () => {
    if (!newSubject.name || !newSubject.marks || !newSubject.totalMarks || !newSubject.credits) return;
  ...
  const updateSubject = (id: string, field: keyof Subject | 'marks' | 'totalMarks' | 'credits' | 'name' | 'grade', value: string) => {
    const updatedSubjects = subjects.map(subject => {
      if (subject.id !== id) return subject;

      // copy so we don't mutate original
      const updated: Subject = { ...subject };

      // update appropriate field
      if (field === 'name') {
        updated.name = value;
      } else if (field === 'marks') {
        updated.marks = parseFloat(value) || 0;
      } else if (field === 'totalMarks') {
        updated.totalMarks = parseFloat(value) || 0;
      } else if (field === 'credits') {
        updated.credits = parseFloat(value) || 0;
      } else if (field === 'grade') {
        updated.grade = value.toUpperCase();
        updated.gradePoints = gradeMap[updated.grade] ?? 0;
      }

      // Recalculate percentage & gradePoints when marks/totalMarks/grade/credits change
      const total = updated.totalMarks ?? 0;
      const mk = updated.marks ?? 0;
      updated.percentage = total > 0 ? Math.round((mk / total) * 10000) / 100 : 0;
      updated.gradePoints = gradeMap[updated.grade] ?? updated.gradePoints ?? 0;
